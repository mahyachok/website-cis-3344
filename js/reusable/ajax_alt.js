...
function ajax_alt (url, successFn, failureFn) {
    ...
    console.log("ready to make ajax call on this url: "+url);
    ...
    function callBack() {
        ...
        if (httpReq.readyState === 4) {
            if (httpReq.status === 200) {
                ...
                var eMsg = "Error: could not read URL '" + url + "'. "+
                        "Must 'Run' not 'View' for AJAX to work.";
                console.log("ajax_alt failure error is " + eMsg);
                console.log(httpReq); // no longer holds valuable status and statusText values.
                failureFn(eMsg);
            }
        }
    } // end of callBack
